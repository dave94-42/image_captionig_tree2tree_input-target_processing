...
class Tree:
    ...
    def write_data_in_leafs(self,data_cnn):
        """
        write file in "current level" leafs
        :return:
        """
        self.data = torch.ones(data_cnn[0].shape)*np.finfo(float).min

        if self.isLeaf == True:
            self.data = data_cnn[self.label-1]
        else:
            for child in self.childs:
                child.write_data_in_leafs(data_cnn)
                child.parent.data = torch.max(child.data,child.parent.data)

        all(isinstance(child,torch.Tensor) for child in self.childs)
        ...
        for child in self.childs:
            if child.isLeaf == True:
                child.data = data_cnn[child.label-1]
                self.data = torch.max(self.data,child.data)
                i=i+1
            else:
                child.write_data_in_leafs(data_cnn)
    ...
    def get_string_from_tree(self):
        ...
        for child in self.childs:
            if child.isLeaf == True:
                info = extract_string_fromTensor( str(child.data) )
...
def extract_string_fromTensor(info):
    """
