...
(TContainer& picks, TTree const& tree,
 std::vector<std::pair<double, double>> const& Ems,  std::vector<std::pair<int , std::pair <int,int>>> &my_tree )
{
    ...
    if (Ems[i].first < Ems[i].second) {
      ...
      for (auto child: tree[i].children) {
        if (printed==false) {
            ...
            tmp.first = i;
            tmp.second.first = tree[i].children[0];
            tmp.second.second = tree[i].children[1];
            my_tree.push_back(tmp);
            printed = true;
        }
      nq.push(child); }
    }
